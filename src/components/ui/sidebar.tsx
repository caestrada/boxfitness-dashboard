...
function SidebarProvider({
  ...
  const setOpen = (nextOpen: boolean) => {
    ...
    // biome-ignore lint/suspicious/noDocumentCookie: Cookie Store API lacks Safari/Firefox support; shadcn sidebar uses document.cookie for persistence.
      ...
      >
        <div
          className={cn(
            ...
            variant === "sidebar" &&
              "rounded-none border-y-0 border-l-0 border-r border-sidebar-border shadow-none",
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-3 border-b border-sidebar-border/70 p-4",
        className,
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "flex w-full items-center gap-3 overflow-hidden rounded-[1.25rem] px-3.5 py-2.5 text-left text-sm transition-[background-color,color,box-shadow,border-color] outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring/20 disabled:pointer-events-none disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      ...
      isActive: {
        true: "surface-control text-foreground shadow-[0_18px_44px_-30px_rgba(15,23,42,0.26)] dark:shadow-[0_24px_56px_-34px_rgba(0,0,0,0.62)]",
        false:
