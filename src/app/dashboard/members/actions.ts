...
function parseMemberFormData(formData: FormData): MemberFormValues {
  return {
    ...
    status: getStringFormValue(formData, "status", "active") as MemberFormValues["status"],
...
function getFieldErrors(
  parsedError: z.ZodError<z.infer<typeof createMemberSchema> | z.infer<typeof updateMemberSchema>>
) {
...
async function updateMemberDirectly(options: {
  ...
  if (existingMemberError || !existingMember) {
    throw new Error(existingMemberError?.message ?? "The member record could not be found.")
  }
...
async function deleteMemberDirectly(options: {
  ...
  if (membershipLookupError || !membershipRow) {
    throw new Error(
      membershipLookupError?.message ?? "The member could not be found in this gym."
    )
  }
...
export async function updateMemberAction(
  ...
  const parsedValue = updateMemberSchema.safeParse({
    ...parseMemberFormData(formData),
    memberId:
      typeof formData.get("memberId") === "string" ? formData.get("memberId") : "",
    membershipId:
