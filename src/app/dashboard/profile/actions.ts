...
export async function updateProfileAvatarAction(
  ...
  if (currentProfileError) {
    return createErrorState(
      getProfilesTableErrorMessage(currentProfileError.message),
    );
  }
  ...
  const currentAvatarUrl =
    ...
    currentProfile.avatar_url.trim().length > 0
      ...
      : null;
  const currentStoragePath = getProfileAvatarStoragePath(
    currentAvatarUrl,
    supabaseUrl,
  );
  ...
  if (intent === "remove") {
    ...
    if (updateError) {
      return createErrorState(
        getProfilesTableErrorMessage(updateError.message),
      );
    }
  ...
  if (avatarFile.size > PROFILE_AVATAR_MAX_BYTES) {
    return createErrorState(
      `Avatar images must be ${PROFILE_AVATAR_MAX_SIZE_LABEL} or smaller.`,
      {
  ...
  const fileExtension = getProfileAvatarFileExtension(
    avatarFile.name,
    avatarFile.type,
  );
...
export async function updateProfileDetailsAction(
  ...
  if (!parsedValue.success) {
    return createProfileDetailsErrorState(
