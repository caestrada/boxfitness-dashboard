...
const cancelSubscriptionBodySchema = z.object({
  organizationId: z.string().uuid("Select a valid workspace before updating the plan."),
})
...
function getManagedSubscriptionForOrganization(
  ...
  return (
    uniqueSubscriptions.sort((left, right) => {
      ...
      const leftManagedStatus = MANAGED_STRIPE_SUBSCRIPTION_STATUSES.has(left.status) ? 1 : 0
      const rightManagedStatus = MANAGED_STRIPE_SUBSCRIPTION_STATUSES.has(right.status) ? 1 : 0
    ...
    }).find((subscription) => MANAGED_STRIPE_SUBSCRIPTION_STATUSES.has(subscription.status)) ?? null
  )
}
...
async function getActiveOrganizationSubscription(
  ...
  return getManagedSubscriptionForOrganization(
    [...subscriptions.data, ...(directSubscription ? [directSubscription] : [])],
    organizationId,
...
export async function POST(request: Request) {
  ...
  let payload: z.infer<typeof cancelSubscriptionBodySchema>

  try {
    payload = cancelSubscriptionBodySchema.parse(await request.json())
  } catch {
    return createErrorResponse(400, "Choose a valid workspace before updating the plan.")
  }
  ...
  if (!organization.stripe_customer_id && !organization.stripe_subscription_id) {
    return createErrorResponse(409, "This workspace does not have a Stripe subscription to update.")
  }
  ...
  try {
    ...
    if (activeSubscription.cancel_at_period_end) {
      return createErrorResponse(409, "This workspace is already scheduled to cancel.")
    }
    ...
    const message = error instanceof Error ? error.message : "The plan could not be updated."
