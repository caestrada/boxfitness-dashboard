...
const cancelSubscriptionBodySchema = z.object({
  organizationId: z
    .string()
    .uuid("Select a valid workspace before updating the plan."),
});
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return createErrorResponse(
      400,
      "Choose a valid workspace before updating the plan.",
    );
  }
  ...
  if (
    ...
    return createErrorResponse(
      409,
      "This workspace does not have a Stripe subscription to update.",
    );
  }
  ...
  try {
    ...
    if (activeSubscription.cancel_at_period_end) {
      return createErrorResponse(
        409,
        "This workspace is already scheduled to cancel.",
      );
    }
