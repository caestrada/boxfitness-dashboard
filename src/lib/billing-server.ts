...
import { type BillingTierKey, normalizeBillingTier } from "@/lib/billing";
...
function getConfiguredStripePlanId(tier: Exclude<BillingTierKey, "free">) {
  ...
  const priceId = tier === "starter" ? stripeStarterPriceId : stripeProPriceId;
  if (!priceId) {
    throw new Error(MISSING_STRIPE_BILLING_ENV_MESSAGE);
  }
...
async function loadRecurringPriceIdForProduct(
  stripe: Stripe,
  productId: string,
) {
...
export async function getStripePriceIdForTier(
  ...
  const resolvedPlanIdPromise = loadRecurringPriceIdForProduct(
    stripe,
    configuredPlanId,
  );
...
function getRelevantStripeSubscription(
  ...
  return (
    uniqueSubscriptions.sort((left, right) => {
      ...
      const leftManagedStatus = isManagedStripeSubscriptionStatus(left.status)
        ? 1
        : 0;
      const rightManagedStatus = isManagedStripeSubscriptionStatus(right.status)
        ? 1
        : 0;
...
function buildSynchronizedBillingSnapshot(
  ...
  const mappedTier =
    getBillingTierFromStripeSubscription(subscription) ??
    getFallbackTierForSnapshot(snapshot);
...
export async function synchronizeOrganizationBillingSnapshot(
  ...
  const synchronizedSnapshot = buildSynchronizedBillingSnapshot(
    snapshot,
...
export function getBillingTierFromStripeSubscription(
  ...
  const mappedTier = getBillingTierForStripePlan(
    primaryPrice?.id,
    getPriceProductId(primaryPrice),
  );
