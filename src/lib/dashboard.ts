...
export function parseDashboardBillingOrganizations(
  ...
  const parsedOrganizations = z
    .array(dashboardBillingOrganizationSchema)
    .safeParse(organizationRows);
  ...
  if (parsedMemberships.success) {
    for (const membership of parsedMemberships.data) {
      ...
      membershipByOrganizationId.set(
...
export function parseDashboardProfile(
  ...
  return {
    email:
      normalizeOptionalString(parsedValue.data.email) ??
      normalizeOptionalString(fallbackEmail),
    fullName: normalizeOptionalString(parsedValue.data.full_name),
    avatarUrl: normalizeOptionalString(parsedValue.data.avatar_url),
    defaultOrganizationId: normalizeOptionalString(
      parsedValue.data.default_organization_id,
    ),
  };
}
...
export function resolveActiveGym<T extends { id: string }>(
  ...
  return (
    gyms.find((gym) => gym.id === defaultOrganizationId) ?? gyms[0] ?? null
  );
}
