...
const memberOrganizationRowSchema = z.object({
  ...
  member: z.union([memberPersonSchema, z.array(memberPersonSchema).max(1)]).nullable().optional(),
})
    ...
    return [
      {
        ...
        outstandingBalanceCents: Math.max(0, row.outstanding_balance_cents ?? 0),
      },
    ]
  })
}
